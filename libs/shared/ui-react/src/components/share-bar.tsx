 ...
 */
export function ShareBar({ url, title, text, locale, className }: ShareBarProps) {
  ...
  const summary = enc(text ? `${title} — ${text}` : title);

  const targets: Array<{ key: string; label: string; href: string; path: string }> = [
    {
      key: 'whatsapp',
      label: 'WhatsApp',
      href: `https://api.whatsapp.com/send?text=${titleEnc}%20${u}`,
    ...
    },
    {
      key: 'pinterest',
      label: 'Pinterest',
      href: `https://pinterest.com/pin/create/button/?url=${u}&description=${summary}`,
